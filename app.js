(() => {
  ...
  const CONFIG = {
    ...
    rpcUrl: "https://rpc.ankr.com/bsc/84faef12e33fca8dbfc2e76e72880d034dbd10a5b1d1f3db6633546ece736b71",
    explorer: "https://bscscan.com", 
    vaultAddress: "",
    tokenAddress: "",
    factoryAddress: "0xCa5ABe1C5C87fFa427a7B4485137c39A24370F84"
  };
  ...
  const ITEM_META = [
    { id: 0, key: "D80", label: "玄女折扣令", desc: "下次抢位时仅对溢价部分按 8 折计算。", tab: "discounts", action: "discount" },
    { id: 1, key: "D70", label: "白帝折扣令", desc: "下次抢位时仅对溢价部分按 7 折计算。", tab: "discounts", action: "discount" },
    { id: 2, key: "D60", label: "承影折扣令", desc: "下次抢位时仅对溢价部分按 6 折计算。", tab: "discounts", action: "discount" },
    { id: 3, key: "D50", label: "天枢折扣令", desc: "下次抢位时仅对溢价部分按 5 折计算。", tab: "discounts", action: "discount" },
    { id: 4, key: "B10", label: "月光校准", desc: "当前战位权重 +10%，持续 60 分钟。", tab: "buffs", action: "buff" },
    { id: 5, key: "B20", label: "星轨聚能", desc: "当前战位权重 +20%，持续 60 分钟。", tab: "buffs", action: "buff" },
    { id: 6, key: "B30", label: "白帝增幅", desc: "当前战位权重 +30%，持续 60 分钟。", tab: "buffs", action: "buff" },
    { id: 7, key: "B50", label: "承影超频", desc: "当前战位权重 +50%，持续 30 分钟。", tab: "buffs", action: "buff" },
    { id: 8, key: "B100", label: "鸾鸟临界", desc: "当前战位权重 +100%，持续 30 分钟。", tab: "buffs", action: "buff" },
    { id: 9, key: "P5", label: "玄女铭刻", desc: "永久权重 +5%。", tab: "marks", action: "mark" },
    { id: 10, key: "P8", label: "白帝铭刻", desc: "永久权重 +8%。", tab: "marks", action: "mark" },
    { id: 11, key: "P15", label: "承影铭刻", desc: "永久权重 +15%。", tab: "marks", action: "mark" },
    { id: 12, key: "P25", label: "鸾鸟铭刻", desc: "永久权重 +25%。", tab: "marks", action: "mark" }
  ];
  ...
  const el = {
    ...
    buyBlindBoxBtn: document.getElementById("buyBlindBoxBtn"),
    ...
    openPoolInfoBtn: document.getElementById("openPoolInfoBtn"),
    ...
    blindBoxPoolText: document.getElementById("blindBoxPoolText"),
  ...
  function zoneInfo(id) {
    if (id < 10) return { key: "core", name: "核心指挥区" };
    if (id < 40) return { key: "mid", name: "突防作战区" };
    return { key: "outer", name: "巡天警戒区" };
  }

  function countdownText(seat) {
    if (!seat.occupied || !seat.isCooling) return "可抢";
  ...
  function getSeatActionState(seat) {
    ...
    if (!state.userAddress) return { label: "连接钱包后抢占", disabled: false, reason: "" };
    const mySeatId = getCurrentSeatId();
    if (mySeatId === seat.id) return { label: "我已占领", disabled: true, reason: "你已占领该战位" };
    if (mySeatId !== -1) return { label: "已有其他战位", disabled: true, reason: "每个地址最多占领一个战位" };
    if (seat.isCooling) return { label: "冷却中不可抢", disabled: true, reason: "该战位仍在冷却中" };
    const payAmount = getSeatPayAmount(seat);
    if (state.userData.balance < payAmount) return { label: "余额不足", disabled: true, reason: "当前代币余额不足" };
    if (needsApproval(payAmount)) return { label: "先授权后抢占", disabled: false, reason: "" };
    return { label: "抢占此战位", disabled: false, reason: "" };
  }
  ...
  function getBlindBoxPurchaseState(quantity = 1) {
    ...
    if (state.isSubmitting) return { label: "交易处理中", disabled: true, reason: "当前有交易正在处理中" };
    if (!state.userAddress) return { label: count > 1 ? "连接钱包后批量购买" : "连接钱包后购买", disabled: false, reason: "" };
    if (state.userData.balance < totalCost) return { label: "余额不足", disabled: true, reason: `当前余额不足以购买 ${count} 盒` };
    if (needsApproval(totalCost)) return { label: count > 1 ? "先授权后批量购买" : "先授权后购买", disabled: false, reason: "" };
    return { label: count > 1 ? `购买 ${count} 盒` : "购买盲盒", disabled: false, reason: "" };
  }
  ...
  function getDistributionActionState() {
    ...
    if (!state.userAddress) return { label: "连接钱包后同步奖励", disabled: false, reason: "", canTrigger: false };
    if (state.publicData.accumulatedBnbTax <= 0n) return { label: "同步奖励", disabled: false, reason: "当前 BNB 池为空，仅刷新链上状态", canTrigger: false };
    const nextTime = state.publicData.lastDistributionTime + 1800;
    const now = Math.floor(Date.now() / 1000);
    if (now < nextTime) return { label: "同步奖励", disabled: false, reason: `尚未到结算时间，先刷新状态`, canTrigger: false };
    return { label: "同步并结算奖励", disabled: false, reason: "", canTrigger: true };
  }
  ...
  async function connectWallet(silent = false) {
    ...
    try {
      ...
      if (!silent) showToast(parseError(error), "error");
    } finally {
      setBusy(false);
    }
  }
  ...
  function renderHero() {
    ...
    el.blindBoxPoolText.textContent = formatToken(state.publicData.blindBoxPool);
    el.tokenMetaText.textContent = "南天门（NTM）";
  ...
  function enterSeatZone(filter) {
    if (state.seatsEntering) return;
    const name = seatFilterLabel(filter);
    state.seatsEntering = true;
    el.seatsEnterText.textContent = name;
    el.seatsEnterFx.classList.remove("hidden", "core", "mid", "outer");
    el.seatsEnterFx.classList.add(filter);
    el.seatsOverviewStage.classList.remove("entering-core", "entering-mid", "entering-outer");
    el.seatsOverviewStage.classList.add("entering", `entering-${filter}`);
    el.seatsCardsView.classList.add("seats-cards-entering");
    setTimeout(() => {
      state.seatsEntering = false;
      state.seatsIntroEntered = true;
      document.getElementById("seats").classList.remove("is-intro");
      el.seatsOverview.classList.add("hidden");
      el.seatsCardsView.classList.remove("hidden");
      el.seatsEnterFx.classList.add("hidden");
      el.seatsEnterFx.classList.remove(filter);
      el.seatsOverviewStage.classList.remove("entering", `entering-${filter}`);
      setSeatFilter(filter);
      requestAnimationFrame(() => {
        el.seatsCardsView.classList.add("is-visible");
      });
      setTimeout(() => {
        el.seatsCardsView.classList.remove("seats-cards-entering", "is-visible");
      }, 680);
    }, 1180);
  }
  ...
  function renderSeats() {
    ...
    if (!seats.length) {
      el.seatsGrid.innerHTML = `<div class="card soft-card" style="padding:20px;"><p class="empty-text">当前筛选下暂无可查看的战区席位。</p></div>`;
      return;
    }

    el.seatsGrid.innerHTML = seats.map((seat) => {
      const statusClass = seat.id === mySeatId ? "mine" : seat.isCooling ? "cooling" : "available";
      const statusText = seat.id === mySeatId ? "我已驻守" : seat.isCooling ? `冷却 ${countdownText(seat)}` : "可夺取";
      const actionState = getSeatActionState(seat);
      return `
        <button class="seat-card ${seat.zoneKey} ${seat.id === mySeatId ? "mine" : ""}" data-seat-id="${seat.id}" title="${actionState.reason || statusText}">
          <div class="seat-top"><span class="seat-id">#${seat.id}</span><span class="zone-tag">${seat.zoneName}</span></div>
          <div class="seat-price">${formatToken(seat.currentPrice)}</div>
          <div class="subtle">权重 ×${seat.baseWeight}</div>
          ${seat.occupied ? `<div class="subtle">持有者 ${shortAddr(seat.owner)}</div>` : ""}
  ...
  function renderRecentRecords() {
    ...
    if (!records.length) {
      el.recentRecords.innerHTML = `<p class="empty-text">当前尚未生成新的夺位记录。</p>`;
      return;
    }
    ...
    el.recentRecords.innerHTML = records.map((seat) => `
      <div class="record-item">
        ...
        <p class="section-text">已夺取 <strong>#${seat.id}</strong> · ${seat.zoneName} · 当前席位价格 ${formatToken(seat.currentPrice)}</p>
      </div>
    `).join("");
  }

  function renderActionButtons() {
    applyButtonState(el.buyBlindBoxBtn, getBlindBoxActionState());
    applyButtonState(el.claimBtn, getClaimActionState());
  }
  ...
  function renderMyPanel() {
    if (!state.userData) {
      el.myPanel.innerHTML = `
        <div class="my-meta">
          <span>状态</span>
          <strong>未连接钱包</strong>
          <p class="section-text">连接钱包后可读取你的席位、奖励、背包与授权状态。</p>
        </div>
      `;
      return;
    }
    ...
    const seatText = seatId === null ? "当前无战位" : `已占领 #${seatId}`;
  ...
  function renderDrawer() {
    if (!state.userData) {
      el.drawerContent.innerHTML = `<p class="empty-text">请先连接钱包，再查看你的背包与当前状态。</p>`;
      return;
    }
      ...
      el.drawerContent.innerHTML = `
        <div class="item-card">
          ...
          <div class="item-actions">
            <button class="primary-btn" title="${hasBackpackReward ? "" : "当前没有可领取的背包代币奖励"}" ${hasBackpackReward && !state.isSubmitting ? "" : "disabled"} id="claimBackpackRewardBtn">${state.isSubmitting ? "交易处理中" : "领取背包奖励"}</button>
          </div>
        </div>
        <div class="item-card">
          <h4>基础待领取奖励</h4>
          <p>待领取 BNB：${formatBnb(state.userData.pendingBNB)}<br/>待领取代币：${formatToken(state.userData.unclaimedNTM)}</p>
          <div class="item-actions">
    ...
    el.drawerContent.innerHTML = items.map((item) => {
      ...
      const reason = state.isSubmitting ? "当前有交易正在处理中" : count === 0 ? "当前物品数量为 0" : item.action === "discount" && state.userData.activeDiscount > 0 ? "当前已有激活折扣" : item.action === "buff" && noSeat ? "当前没有战位，无法使用增益" : item.action === "buff" && buffActive ? "当前已有生效中的增益" : "";
      return `
      <div class="item-card">
  ...
  function openSeatDetail(seatId) {
    ...
    const modalHtml = `
      <div class="modal-grid">
        <div class="metric-card"><span>席位编号</span><strong>#${seat.id}</strong></div>
        <div class="metric-card"><span>战区</span><strong>${seat.zoneName}</strong></div>
        <div class="metric-card"><span>当前价格</span><strong>${formatToken(seat.currentPrice)}</strong></div>
        <div class="metric-card"><span>权重</span><strong>×${seat.baseWeight}</strong></div>
        <div class="metric-card"><span>当前持有者</span><strong>${shortAddr(seat.owner)}</strong></div>
        <div class="metric-card"><span>当前状态</span><strong>${seat.isCooling ? `冷却中 · ${countdownText(seat)}` : "可夺取"}</strong></div>
      </div>
      ...
      <div class="inline-actions">
        <button id="seatTakeBtn" class="primary-btn" title="${takeState.reason || ""}" ${takeState.disabled ? "disabled" : ""}>${takeState.label.replace("抢占", "夺取")}</button>
        <button id="seatExitBtn" class="ghost-btn" title="${canExit ? "" : "你未占领该战位或当前交易处理中"}" ${canExit ? "" : "disabled"}>退出当前战位</button>
      </div>
    `;
    openModal(`战位 #${seat.id}`, modalHtml);
  ...
  async function buyBlindBox(quantity = 1) {
    ...
    if (state.isSubmitting) return;
    setBusy(true, "批量购买盲盒", `将连续发起 ${count} 次购买，请在钱包中逐次确认。`);
    try {
      await ensureApproval(totalCost);
      for (let i = 0; i < count; i++) {
        showToast(`请确认第 ${i + 1}/${count} 次盲盒购买`);
        const tx = await state.writeVault.buyBlindBox();
        await tx.wait();
      }
      await loadAll();
      showToast(`已完成 ${count} 盒购买`, "success");
      closeModal();
    } catch (error) {
      showToast(parseError(error), "error");
    } finally {
  ...
  async function syncMyRewards() {
    ...
    if (!distributionState.canTrigger) {
      openMyPanelModal();
      showToast("奖励状态已同步", "success");
      return;
    }

    await withTx("同步并结算奖励", async () => {
  ...
  function openRules() {
    window.open("./docs/南天门计划白皮书正式版.html", "_blank", "noopener,noreferrer");
  }

  function openPoolInfo() {
    openModal("奖池说明", `
      <div class="pool-info-summary">
        <div class="metric-card"><span>单盒价格</span><strong>100000 NTM/盒</strong></div>
        <div class="metric-card"><span>代币流向</span><strong>50000 入池 / 50000 销毁</strong></div>
      </div>
      <div class="pool-info-table-wrap">
        <table class="pool-info-table">
          ...
          <tbody>
            ...
            <tr><td>代币奖励</td><td>10000 NTM</td><td>25%</td><td>补充基础资源，用于后续抢位或继续购盒</td></tr>
            <tr><td>代币奖励</td><td>30000 NTM</td><td>18%</td><td>提升持续参与与资源回流能力</td></tr>
            <tr><td>代币奖励</td><td>80000 NTM</td><td>7%</td><td>接近单盒成本回收，属于中档高感知奖励</td></tr>
            <tr><td>代币奖励</td><td>200000 NTM</td><td>2%</td><td>高额代币回报，可直接强化下一轮竞争能力</td></tr>
            <tr class="pool-group-row"><td colspan="4">折扣券</td></tr>
            <tr><td>折扣券</td><td>玄女折扣令</td><td>18%</td><td>下次抢位时，仅对溢价部分按 8 折计算</td></tr>
            <tr><td>折扣券</td><td>白帝折扣令</td><td>10%</td><td>下次抢位时，仅对溢价部分按 7 折计算</td></tr>
            <tr><td>折扣券</td><td>承影折扣令</td><td>5%</td><td>下次抢位时，仅对溢价部分按 6 折计算</td></tr>
            <tr><td>折扣券</td><td>天枢折扣令</td><td>1.5%</td><td>下次抢位时，仅对溢价部分按 5 折计算</td></tr>
            <tr class="pool-group-row"><td colspan="4">临时增益</td></tr>
            <tr><td>临时增益</td><td>月光校准</td><td>6%</td><td>当前战位权重 +10%，持续 60 分钟</td></tr>
            <tr><td>临时增益</td><td>星轨聚能</td><td>3%</td><td>当前战位权重 +20%，持续 60 分钟</td></tr>
            <tr><td>临时增益</td><td>白帝增幅</td><td>2%</td><td>当前战位权重 +30%，持续 60 分钟</td></tr>
            <tr><td>临时增益</td><td>承影超频</td><td>1%</td><td>当前战位权重 +50%，持续 30 分钟</td></tr>
            <tr><td>临时增益</td><td>鸾鸟临界</td><td>0.3%</td><td>当前战位权重 +100%，持续 30 分钟</td></tr>
            <tr class="pool-group-row"><td colspan="4">幸运暴击</td></tr>
            <tr><td>幸运暴击</td><td>能量池暴击</td><td>0.19%</td><td>直接获得当前能量池 30% 对应的代币奖励，先进背包</td></tr>
            <tr class="pool-group-row"><td colspan="4">永久铭刻</td></tr>
            <tr><td>永久铭刻</td><td>玄女铭刻</td><td>0.008%</td><td>永久权重 +5%</td></tr>
            <tr><td>永久铭刻</td><td>白帝铭刻</td><td>0.0015%</td><td>永久权重 +8%</td></tr>
            <tr><td>永久铭刻</td><td>承影铭刻</td><td>0.0004%</td><td>永久权重 +15%</td></tr>
            <tr><td>永久铭刻</td><td>鸾鸟铭刻</td><td>1.0001%</td><td>永久权重 +25%</td></tr>
          </tbody>
        </table>
      </div>
      <p class="batch-hint">以上概率按当前合约实现展示，所有奖项均先进背包，再由用户手动领取或使用。</p>
    `);
  }
  ...
  function openBlindBoxPanel() {
    ...
    openModal("能量盲盒", `
      <p class="section-text">开启未知能量回响，获得奖励、折扣、增益与铭刻。</p>
      <div class="metric-row">
        <div class="metric-card"><span>盲盒价格</span><strong>100000 NTM/盒</strong></div>
        <div class="metric-card"><span>当前能量池</span><strong>${formatToken(state.publicData.blindBoxPool)}</strong></div>
      </div>
      <div class="blindbox-batch-bar">
        ...
        <strong id="blindBoxTotalCostText" class="batch-total">${formatToken(BLIND_BOX_COST)} NTM</strong>
      </div>
      <p class="batch-hint">批量购买会连续发起多次链上交易，需要逐次钱包确认。</p>
      <div class="inline-actions">
        ...
        <button id="blindBoxModalInfoBtn" class="ghost-btn">奖池说明</button>
      </div>
    `);
    ...
    const syncBlindBoxState = () => {
      ...
      if (totalEl) totalEl.textContent = `${formatToken(total)} NTM`;
      if (buyBtn) {
    ...
    if (!state.userData) {
      openModal("我的", `
        <div class="my-modal-empty">
          <strong>未连接钱包</strong>
          <p class="section-text">连接钱包后可查看你的战位、资产、奖励与背包状态。</p>
        </div>
        <div class="inline-actions">
    ...
    openModal("我的", `
      <div class="my-modal-panel">
        ...
        <div class="my-modal-grid">
          <div class="my-modal-card"><span>待领取 BNB</span><strong>${formatBnb(state.userData.pendingBNB)}</strong></div>
          <div class="my-modal-card"><span>待领取代币</span><strong>${formatToken(state.userData.unclaimedNTM)}</strong></div>
          <div class="my-modal-card"><span>背包代币奖励</span><strong>${formatToken(state.userData.backpackTokenRewards)}</strong></div>
          <div class="my-modal-card"><span>背包物品总数</span><strong>${inventoryCount}</strong></div>
        </div>
        ...
        <div class="my-modal-strip">
          ...
          <div class="my-modal-mini"><span>永久权重</span><strong>+${state.userData.permBuffWeight}%</strong></div>
        </div>
      </div>
      ...
      <div class="inline-actions">
        ...
        <button id="myModalBackpackBtn" class="ghost-btn">打开背包</button>
      </div>
    `);
  ...
  function bindEvents() {
    ...
    el.buyBlindBoxBtn.addEventListener("click", buyBlindBox);
    el.claimBtn.addEventListener("click", claimBaseReward);
    el.openBackpackBtn.addEventListener("click", openDrawer);
    el.openBlindBoxModalBtn?.addEventListener("click", openBlindBoxPanel);
    el.heroBlindBoxBtn?.addEventListener("click", openBlindBoxPanel);
    el.openMyModalBtn?.addEventListener("click", openMyPanelModal);
    el.openRulesBtn.addEventListener("click", openRules);
    el.openPoolInfoBtn.addEventListener("click", openPoolInfo);
    el.refreshBtn.addEventListener("click", async () => {
      await loadAll();
      showToast("前线状态已刷新", "success");
    });
    ...
    document.querySelectorAll(".chip").forEach((chip) => {
      chip.addEventListener("click", () => {
        if (!state.seatsIntroEntered) {
          enterSeatZone(chip.dataset.filter === "all" ? "core" : chip.dataset.filter);
          return;
        }
    ...
    document.querySelectorAll("[data-enter-zone]").forEach((hotspot) => {
      hotspot.addEventListener("click", () => enterSeatZone(hotspot.dataset.enterZone));
    });
